				...
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">×</button>
						<h4 class="modal-title" id="myModalLabel">
						<button type="button" onclick="pay_zfb2()" style="width: 23%" class="btn btn-default" >支付宝
						</button>
						<button type="button" onclick="pay_wx2()" style="width: 23%" class="btn btn-default" >微信
						</button>
						<button type="button" onclick="pay_cash2()" style="width: 23%" class="btn btn-default" >现金
						</button>
						<button type="button" onclick="pay_card2()" style="width: 23%" class="btn btn-default" >停车卡
						</button>
						</h4>
					</div>
					<div class="modal-body1">
					<div id="pay_zfb2">
					<h4>支付宝：<span id="zfb_text2"></span>元</h4>
					<img alt="" style="width: 300px;height: 300px;margin-left: 20%" src="${APP_PATH }/pic/zfb.png">
					</div>
					<div id="pay_wx2" hidden="hidden">
					<h4>微信：<span id="wx_text2"></span>元</h4>
					<img alt="" style="width: 300px;height: 300px;margin-left: 20%" src="${APP_PATH }/pic/wx.png">
					</div>
					<div id="pay_cash2" hidden="hidden">
					<h4>现金：<span id="cash_text2"></span>元</h4>
					</div>
					<div id="pay_card2" hidden="hidden">
					<h4>停车卡：<span id="card_text2"></span>元</h4>
					</div>
					</div>
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">关闭
						</button>
						<button id="paySubmit2" type="button" onclick="" class="btn btn-primary">支付</button>
					</div>
				</div>
				<!-- /.modal-content -->
			</div>
...
$(document).ready(function(){
	...
	function checkOut(parknum) {
		 $.ajax({
			...
			success:function(data){
				...
				var html = 
				// 用什么方式支付
				"<input id=\"payid\" name=\"payid\" value=\"1\" hidden=\"hidden\"/>"
				// 需要支付金额 
				...
				$("#checkSubmit").attr("onclick","checkOutPay()");
				$(".modal-body").append(html);
				$("#myModal").modal('show');
					}
			}
	...
	function checkOutPay(){
		...
		$("#card_text2").text(20);
		$("#pay_money").val(20);
		$("#paySubmit2").attr("onclick","checkOutSubmit()");
	}
	...
	function illegalSubmit()
	{
		$.ajax({
			...
			success:function(data){
				...
				if(data.code==100)
					{
					var illcount = data.extend.illcount;
					alert(data.extend.va_msg+",该停车卡已违规"+illcount+"次");
				$("#myModal").modal('hide');
				window.location.href="/depot-system/index/toindex";
					}else{
					alert(data.extend.va_msg)
